        ...
        return (
          <button
            ...
            className={`snap-start relative flex items-center gap-2 px-3.5 py-2 text-xs font-semibold rounded-2xl transition-all duration-200 shrink-0 border min-h-[40px] ${
              isSelected
                ? 'text-white bg-brand-gradient border-brand-blue/60 shadow-glow'
                : 'text-slate-300 bg-surface/80 hover:bg-card hover:text-white border-slate-800/80'
            }`}
          >
            <IconComponent
              className={`w-3.5 h-3.5 transition-colors ${
                isSelected ? 'text-white' : 'text-brand-blue-light'
              }`}
            />
            <span>{cat.label}</span>
            {isSelected && (
              <motion.div
                ...
                className="absolute inset-0 rounded-2xl bg-brand-blue/20 border border-brand-blue/40"
