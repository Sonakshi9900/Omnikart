...
export default function MainLayout({ children, searchQuery, setSearchQuery, onOpenCategories }) {
  ...
  return (
    <div className="min-h-screen w-full bg-[#050811] text-slate-100 flex flex-col font-sans text-sm overflow-x-hidden selection:bg-indigo-500 selection:text-white">
      ...
      <header className="block md:hidden sticky top-0 z-40 bg-[#050811]/90 backdrop-blur-xl border-b border-slate-800/80 px-4 py-2.5">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex-1 relative">
            <div className="relative flex items-center bg-slate-900 border border-slate-800 rounded-xl px-3 py-1.5 focus-within:border-indigo-500">
          ...
          <button
            ...
            className="relative p-2 rounded-xl bg-slate-900 border border-slate-800 text-slate-300 active:scale-95 transition-all shrink-0 min-h-[44px] min-w-[44px] flex items-center justify-center"
          >
            <ShoppingBag className="w-5 h-5 text-indigo-400" />
            {cartCount > 0 && (
              <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-indigo-600 text-white font-bold text-[10px] flex items-center justify-center border border-[#050811]">
                {cartCount}
              </span>
            )}
          </button>
