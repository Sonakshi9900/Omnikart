...
export default function BottomNavBar({ onOpenCategories }) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-surface/95 backdrop-blur-xl border-t border-slate-800/90 md:hidden pb-safe">
      <div className="grid grid-cols-4 h-16 max-w-lg mx-auto px-2">
        ...
        <Link
          ...
          className={`flex flex-col items-center justify-center min-h-[44px] min-w-[44px] text-xs font-semibold transition-colors active:scale-95 ${
            isActive('/') ? 'text-brand-blue-light font-bold' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
        ...
        >
          <div className="relative">
            <ShoppingBag className="w-5 h-5 text-brand-orange-light mb-0.5" />
            {cartCount > 0 && (
              <span className="absolute -top-2 -right-2.5 w-4 h-4 rounded-full bg-brand-orange text-white font-extrabold text-[9px] flex items-center justify-center border border-background">
                {cartCount}
              </span>
            )}
          </div>
        ...
        >
          {user ? (
            <img
              ...
              alt={user.name}
              className="w-5 h-5 rounded-full border border-brand-blue-light object-cover mb-0.5"
            />
          ) : (
