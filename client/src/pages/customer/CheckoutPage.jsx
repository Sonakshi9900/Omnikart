      ...
      <div className="min-h-screen bg-background text-slate-100 flex flex-col">
        ...
        <main className="flex-1 flex items-center justify-center p-4">
          <div className="max-w-md w-full glass-panel p-8 rounded-3xl border border-slate-800 text-center space-y-4">
            <div className="w-16 h-16 rounded-full bg-brand-blue/20 text-brand-blue-light border border-brand-blue/40 flex items-center justify-center mx-auto">
              <CheckCircle2 className="w-10 h-10 animate-bounce text-brand-blue-light" />
            </div>
            ...
            <button
              onClick={() => navigate('/')}
              className="px-6 py-2.5 rounded-xl bg-brand-gradient hover:opacity-90 text-white font-bold text-xs shadow-glow transition-all"
            >
    ...
    <div className="min-h-screen bg-background text-slate-100 flex flex-col">
      ...
      <main className="max-w-6xl mx-auto px-4 lg:px-8 py-8 flex-1 w-full space-y-8">
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-2xl bg-brand-blue/20 text-brand-blue-light">
        ...
        <form onSubmit={handleCheckoutSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="glass-panel p-6 rounded-3xl border border-slate-800 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <input
                  ...
                  className="bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-xs text-slate-200 focus:outline-none focus:border-brand-blue"
                />
                <input
                  type="text"
                ...
                <input
                  ...
                  className="sm:col-span-2 bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-xs text-slate-200 focus:outline-none focus:border-brand-blue"
                />
                <input
                  type="text"
                  placeholder="City"
            ...
            <div className="glass-panel p-6 rounded-3xl border border-slate-800 space-y-4">
              <h3 className="font-bold text-slate-100 flex items-center gap-2">
                <CreditCard className="w-5 h-5 text-brand-orange-light" />
              ...
              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-3">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="text-brand-blue-light font-mono font-bold">•••• •••• •••• 4242</span>
                </div>
            ...
            <div className="glass-panel p-6 rounded-3xl border border-slate-800 space-y-4">
              ...
              <div className="space-y-4 max-h-60 overflow-y-auto pr-1">
                {Object.entries(itemsByVendor).map(([vendorName, vendorItems]) => (
                  <div key={vendorName} className="space-y-2 bg-slate-950/60 p-3 rounded-xl border border-slate-800/80">
                    <div className="flex items-center gap-1.5 text-xs font-semibold text-brand-blue-light">
              ...
              <div className="border-t border-slate-800 pt-3 space-y-2 text-xs">
                ...
                <div className="flex justify-between text-slate-400">
                  ...
                  <span className="text-brand-blue-light font-bold">${(totalPrice * 0.1).toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-slate-400">
                  <span>Vendor Direct Payout (90%)</span>
                  <span className="text-brand-orange-light font-bold">${(totalPrice * 0.9).toFixed(2)}</span>
                </div>
                <div className="flex justify-between border-t border-slate-800 pt-2 text-sm font-extrabold text-white">
                  <span>Total Due</span>
                  <span className="text-brand-orange font-bold">${totalPrice.toFixed(2)}</span>
                </div>
              </div>
              ...
              <button
                ...
                className="w-full py-3.5 rounded-xl bg-brand-gradient hover:opacity-90 text-white font-bold text-xs shadow-glow transition-all flex items-center justify-center gap-2"
              >
